...
async def analyze(request: AnalyzeRequest):
    try:
        ...
        recommendations = await get_recommendations(
            repo_data["issues"], graph_json, request.user_profile.dict()
        )
        result = {
            "metadata": {
...
async def match_issues_endpoint(request: MatchIssuesRequest):
    try:
        ...
        matches = await match_issues(
            repo_data["issues"], graph_json, request.user_profile.dict(),
            max_results=request.max_results, label_filter=request.label_filter,
        )
        result = {
            "matches": matches,
        ...
        if not target_issue:
            ...
            for issue in issues:
                if issue.get("number") == request.issue_number:
                    ...
                    break
        if not target_issue:
            raise HTTPException(status_code=404, detail=f"Issue #{request.issue_number} not found")
        graph = build_graph(repo_data["file_tree"], repo_data["file_contents"])
        graph_json = export_graph(graph)
...
async def generate_path_endpoint(request: GeneratePathRequest):
    try:
        ...
        result = await generate_path(
            issue=target_issue, graph=graph,
            file_contents=repo_data["file_contents"],
